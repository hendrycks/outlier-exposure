...
parser = argparse.ArgumentParser(description='Evaluates the PED with a WRN on CIFAR',
...
print('\n\nError Detection')
show_performance(wrong_score, right_score, method_name='OE')
...
ood_data = torch.from_numpy(
    np.random.binomial(n=1, p=0.5, size=(ood_num_examples, 3, 32, 32)).astype(np.float32)) * 2 - 1
ood_data = torch.utils.data.TensorDataset(ood_data, dummy_targets)
